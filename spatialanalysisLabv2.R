...
points<-sites[,4:5] ##assign lat long points for each site to be plotted

## Let's make a map with a background   
library(ggmap)
map<-qmap(location=c(lon = -72.897343, lat = 44.351406),zoom=9, 
          maptype="toner-lite",source="stamen") # map type has a catelogue of
# many types, such as road map, satelite, etc

map + geom_point(data = points, aes(x = Longitude, y = Latitude), 
...
points(x=points, pch=17, col="red") # pch changes shapes
...
bioclimLC <- stack("bioclim-landuse1.tif") # we need to 'stack' the layers on 
#top of each other so that it is one file
names(bioclimLC) <- c("landcover","bio1","bio2","bio3","bio4","bio5","bio6",
                      "bio7","bio8","bio9","bio10","bio11","bio12","bio13",
                      "bio14","bio15","bio16","bio17","bio18","bio19") #renaming
bioclimLC1 <- crop(x=bioclimLC, extent(-74,-71, 42.5, 45.5)) # cropping it to 
#Vermont's extent, x-min, y-min, x-max, y-max in terms of at long
plot(bioclimLC1)

set.seed(44) 
points<-sites[,4:5] # subset only the latitudes and longitudes
fakePres<-sample(c(rep("Y", 9),rep("N",9))) ## creating fake presence data to 
#compare sites
points<-cbind(points, fakePres)
climData <- extract(x=bioclimLC1, y=points[, c("Longitude", "Latitude")]) 
# extracting va
summary(aov(formula=bio1~fakePres, data=dbio1)) # comparing the annual mean 
#values of each layer (19 bioclim, 1 landcover) at each of the sites
dbio1 <- cbind(points, climData) 
#temperature (bio1) of the sites 'with disease' and without
...
fakeP <- ifelse(dbio1$fakePres=="Y", 1, 0) # assigning presence as '1' and 
#absence as '0' for the model
dbio1$fakeP<-fakeP # adding our new column of 0's and 1's to the original 
#climate dataset
MyModel <- glm(fakeP~bio1, family=binomial("logit"),data=dbio1)
summary(MyModel) 

plot(x=dbio1$bio1,y=fakeP,xlab="Annual Mean Temperature (C)",ylab="P(Extant)", 
     type='n', ylim=c(0,1))
...
wSiteAbund$lat<-sites$Latitude # adding longitude and latitude data to site 
#abundance
wSiteAbund$lon<-sites$Longitude
wSiteAbund[is.na(wSiteAbund)]<-0 # replacing NAs with zeros
Names<-names(wSiteAbund)[2:12] # leaving out C. picta because there was only one 
#occurrence


library(rworldmap)
plot(bioclimLC1$bio2, 5, xlim=c(-73.5,-72.4), ylim=c(43,45),col="white", 
     axes=TRUE, legend=F,main="",box=FALSE) # plotting an empty map to get axes
map("state", c('vermont'), interior=T, fill=T, col="lightgreen",add=T)
pies<-mapPies(dF=wSiteAbund[,-4],nameX="lon",nameY="lat",nameZs=Names[-3],
              zColours=c("blue","purple","orange","red","darkred","darkorchid1",
                         "darksalmon","goldenrod","cadetblue","chocolate",
                         "coral4"),xlim=c(-72,-74), ylim=c(43,44.8),
              landCol="gray50",addCatLegend = T,add=T,font=6,symbolSize=2.5)
